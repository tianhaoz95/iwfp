export const cardCollectionPath: string = "cards";
export const noAuthMsg: string = "User not signed in";
export const requestDataIncomplete: string = "Request data incomplete";
export const creditCardAlreadyExistError: string = "Credit card already exist";
export const creditCardNotExistError: string = "Credit card not exist";
export const notImplementedError: string = "Not implemented";
export const promoAlreadyExistError: string = "Promotion already existed";
export const noCardFoundError: string = "No card found";
// Followings are newly added
export const TestUserUid: string = "test_user";
